        ...
        if (title.trim() === "") {
            ...
            const searchMovies = async () => {
                try {
                    ...
                    setMovies(data); // Update movie list with search results
                } catch (error) {
                    console.error("Invalid input", error);
                    setMovies([]); // Clear movie list if search fails
                }
            };

            // Add a debounce effect (delay execution) to avoid excessive API calls
            const timeout = setTimeout(() => searchMovies(), 500);

            return () => clearTimeout(timeout);
        }
    }, [title]); // Runs effect when `title` changes
